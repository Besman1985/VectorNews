...
Optional but recommended firewall on the server:
...
If your VDS provider has its own firewall or security group, also allow inbound `22`, `80`, and `443` there.


### 2. Point the domain to the server


Before configuring Nginx and SSL, create DNS records for your domain:


- `A` record for `example.com` -> `your_vds_ip`
- optional `A` record for `www.example.com` -> `your_vds_ip`


Verify DNS resolution from your machine:
...
- if you plan to use both `example.com` and `www.example.com`, make sure both resolve to the server before running `certbot`
- the Python API currently allows a single `CLIENT_ORIGIN`, so it is safest to choose one canonical frontend domain, for example only `https://example.com`
...
[FIREBASE_CLIENT_EMAIL=firebase-adminsdk@your-project.iam.gserviceaccount.com](mailto:FIREBASE_CLIENT_EMAIL=firebase-adminsdk@your-project.iam.gserviceaccount.com)
FIREBASE_PRIVATE_KEY="-----BEGIN PRIVATE KEY-----\n...\n-----END PRIVATE KEY-----\n"
FIRESTORE_ADMIN_COLLECTION=admin_users
NEXT_PUBLIC_FIREBASE_API_KEY=...
...
- `USER_ACTION_JWT_SECRET` in web and API must also be identical
- `API_URL` for Next.js server-side requests should point to the internal API URL, usually `http://127.0.0.1:4000`
- `NEXT_PUBLIC_API_URL` should point to the public site domain
- `USER_JWT_SECRET` is required by Next.js middleware for user session cookies
- keep `CLIENT_ORIGIN` equal to the exact public frontend origin, because the Python API does not support multiple allowed origins


### 8. Prepare MySQL


If MySQL is on the same server, install and start it first:
...
Then create the application database and user:
...
If MySQL is hosted on a separate server, set `MYSQL_HOST`, `MYSQL_PORT`, `MYSQL_USER`, `MYSQL_PASSWORD`, and `MYSQL_DATABASE` in `apps/api/.env` accordingly and make sure the remote firewall allows access from the VDS.
...
### 10. Seed the Firestore admin user
...
```bash
cd /var/www/vectornews/app
npm run seed:admin -- --email [admin@example.com](mailto:admin@example.com) --password 'change-me' --name 'Main Admin' --service-account /var/www/vectornews/shared/secrets/firebase-adminsdk.json
```


### 11. Create systemd service for the API
...
### 12. Create systemd service for the web app
...
- run `npm run build` before starting the web service for the first time and after every update
...
Optional cleanup if the default site is still enabled:
...
```bash
certbot --nginx -d example.com -d [www.example.com](https://www.example.com)
```
...
```bash
curl [http://127.0.0.1:4000/health](http://127.0.0.1:4000/health)
curl [http://127.0.0.1:4000/api/v1/health/db](http://127.0.0.1:4000/api/v1/health/db)
...
curl [https://example.com/api/rss.xml](https://example.com/api/rss.xml)
```
...
- If comment or like routes fail for logged in users, verify that `USER_ACTION_JWT_SECRET` matches in both apps
- If account or comment actions behave as unauthorized in the web app, verify that `USER_JWT_SECRET` is set in `apps/web/.env.production`
- If editor picks are empty, verify the Firestore document path and service account permissions
- If `curl http://127.0.0.1:4000/api/v1/health/db` fails, verify MySQL is installed, running, and reachable from the API host
- If SSL issuance fails, verify DNS for all requested domains already points to the VDS
